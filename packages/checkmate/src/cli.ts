...
import { runCheckmate, getCheckmatePlan } from "./runner.js";
...
async function main(): Promise<void> {
  ...
  console.log(
    forceMock
      ? "Running Checkmate in MOCK/DRY-RUN mode (harness smoke — NOT a real model eval)\n"
      : "Running Checkmate (will use API key if present, else mock)\n",
  );
  ...
  for (const caseId of caseIds) {
    ...
    try {
      ...
      console.log(`  mental_model: ${result.mentalModelPath}`);
      console.log(`  findings: ${result.findingsPath}`);
      console.log(`  trajectory: ${result.trajectoryPath}`);
      console.log(`  note: ${result.note}`);

      // Gate: all stages + mental_model present
      if (result.stagesCompleted.length < 6) {
        console.error(`  FAIL: expected 6 stages, got ${result.stagesCompleted.length}`);
        failures += 1;
      }
      try {
        await fs.access(result.mentalModelPath);
      } catch {
        console.error(`  FAIL: mental_model.json missing`);
        failures += 1;
      }

      try {
        const score = await scoreCaseDir(caseDir, result.findingsPath);
        const scorePath = path.join(caseArtifactsDir, "checkmate-score.json");
      ...
      failures += 1;
      console.error(
        `FAIL ${caseId}: ${err instanceof Error ? err.message : err}`,
      );
      console.log("");
    }
  }
  ...
  if (failures > 0) {
    console.error(`Checkmate finished with ${failures} failure(s)`);
    process.exit(1);
  }
}
